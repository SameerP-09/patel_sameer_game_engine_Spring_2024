...
# class copied from Mr. Cozort's code in Github
class Spritesheet:
...
# purpose: player can't move through walls
def collide_with_walls(self, dir, entity):
    ...
    elif dir == 'y':        # if sprite moving vertically
        hits = pg.sprite.spritecollide(self, entity, False)
        if hits:
            if self.vy > 0:
...
# purpose: displays text on window
def draw_text(surface, text, size, position, color, x, y):
    ...
    # uses parameter position to determine where to display text
    if position == 'midtop':
...
def draw_health_bar(surf, x, y, pct):
    ...
    # draws the green rectangle and the white outline
